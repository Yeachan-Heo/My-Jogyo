#!/usr/bin/env python3
"""VibeSci Python Bridge - JSON-RPC 2.0 over NDJSON.
...
def _send_protocol(data: dict) -> None:
    ...
    _protocol_out.write(json.dumps(data, ensure_ascii=False, separators=(',', ':')) + '\n')
    _protocol_out.flush()
...
class ExecutionState:
    ...
    def _initialize_namespace(self):
        ...
        self._namespace = {
            "__name__": "__vibesci__",
            "__doc__": "VibeSci execution namespace",
...
def execute_code(
    ...
    try:
        ...
        with contextlib.redirect_stdout(stdout_capture), \
            ...
            compiled = compile(code, "<vibesci>", "exec")
...
def handle_execute(id: str, params: Dict[str, Any]) -> None:
    ...
    if not code:
        send_response(id, error=make_error(
...
def process_request(line: str) -> None:
    ...
    try:
        ...
        try:
            ...
            send_response(None, error=make_error(
                ERROR_PARSE,
                f"Parse error: {e}"
            ))
            return
        ...
        if request.get("jsonrpc") != JSON_RPC_VERSION:
            send_response(request_id, error=make_error(
                ERROR_INVALID_REQUEST,
        ...
        if not isinstance(params, dict):
            send_response(request_id, error=make_error(
                ERROR_INVALID_PARAMS,
                "Parameter 'params' must be an object"
            ))
            return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='VibeSci Python Bridge')
...
def run_socket_server(socket_path: str) -> None:
    ...
    print(f"[vibesci_bridge] Socket server started at {socket_path}, PID={os.getpid()}", file=sys.stderr)
    sys.stderr.flush()
    
    def shutdown_handler(signum, frame):
        print("[vibesci_bridge] Shutdown signal received", file=sys.stderr)
        sys.stderr.flush()
        server.close()
        if os.path.exists(socket_path):
            os.unlink(socket_path)
    ...
    try:
        ...
        print(f"[vibesci_bridge] Server error: {e}", file=sys.stderr)
...
def handle_socket_connection(conn: socket_module.socket) -> None:
    ...
    try:
        ...
        print(f"[vibesci_bridge] Connection error: {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
    finally:
        try:
            conn.close()
...
def run_stdin_mode() -> None:
    print(f"[vibesci_bridge] Started (stdin mode), PID={os.getpid()}", file=sys.stderr)
    sys.stderr.flush()
    ...
    try:
        ...
        print("[vibesci_bridge] Interrupted", file=sys.stderr)
    except Exception as e:
        print(f"[vibesci_bridge] Fatal error: {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        sys.exit(1)
    finally:
        print("[vibesci_bridge] Shutting down", file=sys.stderr)
        _protocol_out.close()
